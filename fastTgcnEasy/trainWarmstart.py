# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 15:35:31 2026

@author: jthomas48
"""

###############################################################################
#WARMSTART TRAINING FUNCTION
###############################################################################

#implimenting a warm start using a previously trained model
#tutorial on warm start
#https://docs.pytorch.org/tutorials/recipes/recipes/warmstarting_model_using_parameters_from_a_different_model.html
#a lot of this code will be identical to train.py, i will try to notate deviations
...
def fastTgcnWarm(arch, testPath, trainPath, batch_size = 1, k = 32,
    ...
    #NOTES ON WARM START
    #if there are issues, check device issues (ie is something on the gpu while something else on the cpu)
    #if there are issues, check train mode vs test mode, had issues with that in prediction
    #i am including the strict=False in the loading function, but we should try with True as well
    #for now i have the model path fixed but this should really be an arguement
    #chatgpt says: i should move the model to gpu as a whoel after initializing it and loading the saved model into the cpu
    #chatgpt says: there is a difference between training continuation and fine tuning
    #this method of reinitializing optimizer is in line with fine tuning but if we are 
    #continuing the training, we should load the perviously used optimizer (which i dont think we are currently saving)
    #both would be interesting to try
