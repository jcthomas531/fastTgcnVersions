...
\begin{itemize}
    ...
    \item Signature of histograms of orientations (SHOT)
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The models make no use to the graphical structure of the meshes, working purely on point clouds. Could incorporate adjacency matrix.
        \item Color is present in the data and could be used in classification.
    \end{itemize}
    \item Cross validation could be improved by accounting for known sources of dependency.
    \item Improvements to local feature descriptors and other possible descriptors:
    \begin{itemize}
        \item Wanted to use tri-spin images (TriSI) descriptor, however, I could not find an implementation of it.
        \item Using color information in local descriptors. I believe at least SHOT descriptors are capable of this.
        \item There are also things called spectral descriptors that I have ran across, not exactly sure what they are but could be useful.
        \item There are other things listed in \href{https://pcl.readthedocs.io/projects/tutorials/en/master/#features}{features tutorial section} on PCL that I have not investigated.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Local descriptors PFH, FPFG, RoPs, USC, and SHOT all implemented in C++ with timing.
\end{itemize}
...
\begin{itemize}
    ...
    \item I was using PFH to determine how long feature extraction would take at different mesh density levels. It turns out that this is by far the slowest of the feature extraction methods. At 20,000 points, PFH takes about 7 minutes but things like SHOT and USC take about 3 seconds. If we end up seeing higher performance as we mesh density, we can expand our search grid upwards and exclude PFH from the comparison.
    \item The true rugae annotations, annotated in CloudCompare, are labels on the points. In the scan pre-processing steps, the point labels are changed to face labels. There is no express reason for this besides convention. For the segmentation model, labels must be attached to the faces. However, rugae classification is done at the point level. Switching between point and face labels - which I have been doing with a majority rules algorithm - introduces a bit of slop into the process. When scan density is reduced by remeshing, points are re-distributed evenly over the surface.Because of this, there will always need to be some sort of label re-assignment no matter if it was to faces or points so there will always be some imprecision. When scan density is reduced by decimation, the points are simply subset (or this is at least the case for decimate\_pro from pyvista). Thus, the original point labels could be retained all the way to the modeling process. However, the current scan pre-processing pipeline is set up to work with face labels and converting this to work with point labels seems like an unwise use of time for something that is likely only marginally important. This does introduce some slop into the process but it is important to remember that slop is at the "truth" level and wont actually effect model performance. It just means that what we are saying is the rugae region will have some imprecision. Though given the granularity we are working with, I doubt very much that this is perceptible to a dental practitioner and would be within the range of human error anyway. For clarity, the path that labels will take in the decimation process is as follows:
    \begin{enumerate}
        \item Points are labeled in cloud compare
        \item Point labels are changed to face labels in the scan pre-processing
        \item Face labels are changed to point labels to facilitate label retention through the decimation
        \begin{itemize}
            \item This is not a strictly necessary re-labeling, even in the current process. A KD tree could be created with the face labels and queried to by the decimated faces. However, I believe that the conversion at the highest possible granularity is a desirable property of the re-labeling that is superior to using the closest old face label for the new faces. The KD tree and proximity runs a higher risk of mislabeling large new faces. There is risk for the point to face conversion method, however, things stay anchored at the truth a bit better. 
        \end{itemize}
        \item Point labels are changed to face labels to conform to the existing pipeline format
        \item Face labels are changed to point labels for the modeling process.
    \end{enumerate}
    All of the re-labeling above is done via a majority rules algorithm. As I mentioned, all this re-labeling is not strictly necessary for the process to work, but it is the current state of the pipeline. It would be a useful check to see if re-labeling in this way causes general expansion or contraction of the area labeled as rugae region. 
\end{itemize}

\subsection{Possible concerns}

Things that could have gone wrong in the process to check if outcomes are strangely poor.

\begin{itemize}
    \item Matching of labels to points and matching of points to descriptors and matching of descriptors to other descriptors. I know at least the descriptor to descriptor relies on the points being in the same order (c++), though I believe this is reasonable given it is all using a singular data object.
    \item Values in the local descriptor csvs are sometimes stored in scientific notation.
\end{itemize}


\subsection{File structure and organization}

Main directory (for initial test data) for intermediary steps and results is: \\
\texttt{gb\_lss/Thomas/iowaExpTest/rugaePipeOptim} \\
The processed scans (formatted, center, scaled, rotated to master) will be stored at their original locations: \\
\texttt{gb\_lss/Thomas/iowaExpTest/scanData/rugAnnotForm\_cSOriMast} \\
Folders are set up as follows:

\begin{itemize}
    \item \textbf{descriptors:} csv files holding the local feature descriptors for each patient and combo of processing
    \item \textbf{other:} a catch all for holding files that are not part of the primary analysis but are important to the analysis. Currently has the following sub-folders:
    \begin{itemize}
        \item \textbf{surfaceArea:} text files for each patient and combo of processing that contain the surface area of the scan
    \end{itemize}
    \item \textbf{reducedScans:} ply files for each patient and combo of processing that contain density reduced versions of the original rugae annotated scans
    \item \textbf{times:} timing for various steps of the pipeline
\end{itemize}

All folders are organized into the following levels
\begin{itemize}
    \item \texttt{pre/post} 
    \item \texttt{reductionTechniqueXXXXX}, for example \texttt{decim8500} for decimation to 8500 points
    \item files downstream of feature extraction will also have a subfolder \texttt{normRadPropYYYY\_descRadMultZZZZ} for combinations of normal radius proportion parameter $\alpha$ and descriptor radius multiple with values \texttt{YYYY} and \texttt{ZZZ} respectively
    \item files named \texttt{patQQQ[Pre/Post]\_otherinfo} where \texttt{QQQ} is patient number
\end{itemize}
