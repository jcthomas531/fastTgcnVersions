...
loader1 = DataLoader(set1, batch_size=1, shuffle=True, num_workers=1)
...
# ==== PATCH START: load -> CUDA -> TRAIN MODE (BN uses batch stats) ====
state = torch.load(modelPath, map_location="cuda", weights_only=True)
model1.load_state_dict(state)
model1 = model1.cuda()
model1.train()  # IMPORTANT: match training-time test_semseg behavior
print("model.training =", model1.training)
# ==== PATCH END ====
...
########## TRY STRIPPING THIS TO BARE BONES, JUST NECESSITIES
# load in packages
import os
import shutil
import numpy as np
...
loader = loader1
arch = "u"
plyPath="/Shared/gb_lss/Thomas/iowaRme/test1PredC/"
num_classes=17
generate_ply=True
...
for batch_id, (index, points, label_face, label_face_onehot, name, raw_points_face, idx_face) in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
    ...
    # keep tensors on CUDA to match the model device
    coordinate, label_face, centre, idx_face = coordinate.cuda(), label_face.cuda(), centre.cuda(), idx_face.cuda()
    print("d")

    # ==== IMPORTANT: inference in train-mode, but with no gradients ====
    with torch.no_grad():
