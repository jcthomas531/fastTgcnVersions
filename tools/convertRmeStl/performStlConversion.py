...
# #patient specific file names
# preDOutName = "pat" + str(patNumber).zfill(3) + "u_preD.ply"
# finOutName = "pat" + str(patNumber).zfill(3) + "u_fin.ply" 
# #full file paths
# preDOutFilePath = preDOutDir + preDOutName
# finOutFilePath = finOutDir + finOutName
...
# #paths and file names for original stls
# #directory of original files for the patient
# patDir = "K:/iowaRme/preDelivAndFinalScans/originalStl/pat" +  str(patNumber).zfill(3) +"/"
# #find file name of upper scan for pre delivery scan
# preDInNameList = [i for i in os.listdir(patDir) if re.search("u\.stl$", i)]
# #find file name of upper scan for final scane
# finInNameList = [i for i in os.listdir(patDir + "final/") if re.search("u\.stl$", i)]
...
#need logic to skip a directory if number of files meeting the criteria is not 1
#should have this output to a log file
...
#     finInName = finInNameList[0]
#     #append to file path
#     finInFilePath = patDir + finInName
#     #perform conversion to ply
#     csp.convertRmeStlToPly(inFile = finInFilePath,
#                            outFile = finOutFilePath)
# elif len(preDInNameList) == 1:
